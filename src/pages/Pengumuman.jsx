import { Plus, Megaphone, Edit, Trash2, Calendar, X } from 'lucide-react';
...
export default function Pengumuman() {
  ...
  const initialFormState = { judul: '', isi: '', tanggal: today, target: 'Semua', status: 'Aktif' };
  ...
  async function handleAddPengumuman(e) {
    e.preventDefault();
    try {
  ...
  async function handleEditPengumuman(e) {
    e.preventDefault();
    try {
      setIsSubmitting(true);
  ...
  async function handleDeletePengumuman(id) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-16 flex flex-col items-center justify-center text-center shadow-sm">
          ...
          <p className="text-slate-500 max-w-sm mt-1">Buat pengumuman baru untuk memberikan informasi kepada siswa atau guru.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pengumumanData.map((item) => (
            <div key={item.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col h-full hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="p-2 bg-amber-50 text-amber-600 rounded-lg">
                  <Megaphone className="w-6 h-6" />
                </div>
                <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${
                  item.status === 'Aktif' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-600 border-slate-200'
                }`}>
                  {item.status}
                </span>
              </div>
              ...
              <p className="text-sm text-slate-600 mb-4 line-clamp-3">{item.isi || <span className="italic text-slate-400">Tidak ada rincian.</span>}</p>
              ...
              <div className="mt-auto space-y-2 pt-4 border-t border-slate-100">
                ...
                <div className="flex items-center text-sm text-slate-500 gap-2">
                  ...
                  <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-xs font-bold">{item.target}</span>
                </div>
              </div>
              
              <div className="flex justify-end gap-2 mt-4 pt-2">
                <button onClick={() => openEditModal(item)} className="text-blue-600 hover:bg-blue-50 p-2 rounded-lg transition-colors cursor-pointer" title="Edit">
                  <Edit className="w-4 h-4" />
                </button>
                <button onClick={() => handleDeletePengumuman(item.id)} className="text-red-600 hover:bg-red-50 p-2 rounded-lg transition-colors cursor-pointer" title="Hapus">
      ...
      {(isModalOpen || isEditModalOpen) && (() => {
        ...
        const handleSubmit = isEdit ? handleEditPengumuman : handleAddPengumuman;
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
              <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 flex-shrink-0">
                <h2 className="font-bold text-slate-900 text-lg">{isEdit ? 'Edit Pengumuman' : 'Buat Pengumuman Baru'}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
                <div className="space-y-1.5">
                  ...
                  <input type="text" required value={currentData.judul} onChange={(e) => setFunction({...currentData, judul: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Cth: Rapat Orang Tua, Pengumuman Libur" />
                </div>
                
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Isi / Rincian Pengumuman</label>
                  <textarea rows="4" value={currentData.isi} onChange={(e) => setFunction({...currentData, isi: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm resize-none" placeholder="Tuliskan detail pengumuman di sini..."></textarea>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 pt-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700">Tanggal *</label>
                    <input type="date" required value={currentData.tanggal} onChange={(e) => setFunction({...currentData, tanggal: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm cursor-pointer" />
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700">Target *</label>
                    <select required value={currentData.target} onChange={(e) => setFunction({...currentData, target: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm cursor-pointer bg-white">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select required value={currentData.status} onChange={(e) => setFunction({...currentData, status: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm cursor-pointer bg-white">
